...
/datum/ai_planning_subtree/escape_captivity/select_behaviors(datum/ai_controller/controller, seconds_per_tick)
	var/mob/living/living_pawn = controller.pawn

	if (isobj(living_pawn.buckled))
		// we can just stand up we don't need to freak out
		if (pacifist || living_pawn.buckled)
			controller.queue_behavior(/datum/ai_planning_subtree/generic_resist)
	...
	if (!isturf(living_pawn.loc) && !ismob(living_pawn.loc) && !istype(living_pawn.loc))
	...
	if (puller && puller.grabbed_by > GRAB_PASSIVE)
...
/datum/ai_behavior/break_out_of_object/perform(seconds_per_tick, datum/ai_controller/controller, atom/target_atom)
	...
	controller.ai_interact(target = target_atom, INTENT_HARM)
